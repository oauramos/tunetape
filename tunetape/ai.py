...
_SYSTEM = (
    "You are a music curator for a terminal audio player. Given a listener's "
    "request, reply with ONLY a JSON array of up to "
    f"{_MAX_SONGS} real, well-known songs that fit it. Each element must be an "
    'object of the form {"artist": "...", "title": "..."}. Return the raw JSON '
    "array and nothing else — no prose, no markdown, no code fences."
)
...
def _post_messages(key: str, messages: list, *, system: str, max_tokens: int) -> str:
    """POST to the Messages API and return the model's concatenated text reply.
    ...
    Settings connection test so error handling lives in one place.
    """
    body = json.dumps({
        "model": model(),
...
def _call_anthropic(request: str, key: str) -> str:
    """POST a discovery request to the Messages API; return the model's reply."""
...
def suggest_tracks(request: str) -> list:
    ...
    text = _call_anthropic(request, key)
    tracks = _tracks_from_songs(_extract_json_array(text))
    if not tracks:
